...
const char *PlaitsPort::voice_count_names_[] = {"1", "2", "3", "4"};
...
PlaitsPort::PlaitsPort()
    : voice_(nullptr), patch_(nullptr), modulations_(nullptr),
      allocator_(nullptr), voice_count_(1), current_bank_(0),
      midi_note_(kMidiNoteC4), midi_velocity_(0.8f), midi_gate_(false),
      gate_state_(false), previous_gate_(false), previous_gate2_(false),
...
PlaitsPort::~PlaitsPort() {
  ...
  for (int i = 0; i < kMaxPolyVoices - 1; i++) {
    if (poly_voices_[i])
      delete poly_voices_[i];
    if (poly_allocators_[i])
      delete poly_allocators_[i];
  }
}
...
void PlaitsPort::Init(float sample_rate) {
  ...
  for (int i = 0; i < kMaxPolyVoices - 1; i++) {
    ...
    poly_allocators_[i] =
        new stmlib::BufferAllocator(poly_buffers_[i], kBufferSize);
...
void PlaitsPort::RenderPolyphonicVoices(plaits::Voice::Frame *frames,
                                        size_t size) {
  if (!voice_manager_.IsPolyphonyEnabled()) {
    ...
    // Note: Volume parameter modulation is applied separately in Process
    for (size_t i = 0; i < size; i++) {
      frames[i].out = static_cast<short>(frames[i].out * midi_velocity_);
      frames[i].aux = static_cast<short>(frames[i].aux * midi_velocity_);
      frames[i].out_dry =
          static_cast<short>(frames[i].out_dry * midi_velocity_);
      frames[i].aux_dry =
          static_cast<short>(frames[i].aux_dry * midi_velocity_);
    }
  ...
  for (int v = 0; v < voice_count_; v++) {
    ...
    // Get the appropriate voice object (voice_ for v=0, poly_voices_[v-1] for
    // v>0)
    ...
    // Voices are only stolen when needed for new notes (in voice allocation
    // logic).
  }
  ...
  if (active_voices > 0) {
    ...
    for (size_t i = 0; i < size; i++) {
      frames[i].out = static_cast<short>(
          std::clamp(out_accum[i] * gain, -32767.0f, 32767.0f));
      frames[i].aux = static_cast<short>(
          std::clamp(aux_accum[i] * gain, -32767.0f, 32767.0f));
...
void PlaitsPort::Process(float **in, float **out, size_t size) {
  ...
  // Static buffer to avoid stack overflow in audio interrupt (96 frames = 768
  // bytes)
